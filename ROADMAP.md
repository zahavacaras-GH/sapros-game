...
### 🔴 Critical
- [ ] **9 remaining ending images** — Cowork is generating these (worse_than_vespera, vesperas_heir, pyrrhic, mirror, jason, executed, sophie, kaden_kills, vespera)
- [ ] **Paragraph images for the remaining ~120 nodes** — t1.3a–t1.7, all of Acts II–V. Each scene = 4–10 images. Total ≈ 350–400 images. Cowork can do this in batches using `COWORK_INSTRUCTIONS_PARAGRAPHS.md`. You'll request the prompts one scene at a time so visual coherence holds.
- [ ] **Per-paragraph prompts for Acts II–V** — I'll write these in batches as you're ready. Currently only t1.1's prompts exist.
...
### 🟢 Bonus
- [ ] **Visible Corruption gauge tooltip** ("Hover/tap to learn what this means")
...
4. Mobile portrait/landscape audit and fixes (your #5)
5. Move to Vercel (your #6 + #7)
6. Custom domain if you want one

### Phase 3 — Audio
7. Pick 6 mood tracks (suspense / ominous / drama / tender / scared / memory)
8. Tag every node in `story.json` with the right mood (`node.audio`)
9. Optional: audio narration (your #2)
